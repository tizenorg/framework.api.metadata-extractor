...
#define compare_if_return(expr, fmt, val...) do { \
		if(expr) { \
			LOGE(" "fmt"", ##val); 	\
			return (val); \
		} \
...
#define neq_err_none(val, fmt, arg...) do { \
		if (arg != MM_ERROR_NONE) { \
			LOGE(" "fmt"", ##arg); \
			return (val); \
		} \
	} while (0)

#define neq_err_none_free(val, fmt, free, ret...) do { \
...
#define neq_err_none_destroy(val, fmt, free, dest, ret...) do { \
		if (ret!= MM_ERROR_NONE) { \
			LOGE(" "fmt"", ##ret); \
			SAFE_FREE(free); \
			mm_file_destroy_content_attrs(dest);	\
			return (val); \
		} \
	} while (0)
...
static int __metadata_extractor_check_and_extract_meta(metadata_extractor_s *metadata, metadata_extractor_type_e metadata_type)
{
	...
	compare_if_return(!metadata || (!metadata->path && !metadata->buffer), "INVALID_PARAMETER(0x%08x)", METADATA_EXTRACTOR_ERROR_INVALID_PARAMETER);
...
static int __metadata_extractor_create_content_attrs(metadata_extractor_s *metadata, const char *path)
{
	...
	neq_err_none_destroy(METADATA_EXTRACTOR_ERROR_OPERATION_FAILED, "METADATA_EXTRACTOR_ERROR_OPERATION_FAILED(0x%08x)", err_attr_name, content, ret);
...
static int __metadata_extractor_get_duration(metadata_extractor_s *metadata, int *duration)
{
	...
	compare_if_return(!metadata || !metadata->attr_h, "INVALID_PARAMETER(0x%08x)", METADATA_EXTRACTOR_ERROR_INVALID_PARAMETER);

	ret = mm_file_get_attrs(metadata->attr_h, &err_attr_name, MM_FILE_CONTENT_DURATION, &_duration, NULL);
	neq_err_none_free(METADATA_EXTRACTOR_ERROR_OPERATION_FAILED, "METADATA_EXTRACTOR_ERROR_OPERATION_FAILED(0x%08x)", err_attr_name, ret);
...
static int __metadata_extractor_get_audio_track_count(metadata_extractor_s *metadata, int *track_cnt)
{
	...
	compare_if_return(!metadata, "INVALID_PARAMETER(0x%08x)", METADATA_EXTRACTOR_ERROR_INVALID_PARAMETER);
...
static int __metadata_extractor_destroy_handle(metadata_extractor_s *metadata)
{
	...
	if (metadata->attr_h) {
		...
		neq_err_none(METADATA_EXTRACTOR_ERROR_OPERATION_FAILED, "METADATA_EXTRACTOR_ERROR_OPERATION_FAILED(0x%08x)", ret);
	}
...
int metadata_extractor_create(metadata_extractor_h *metadata)
{
	...
	compare_if_return(_metadata == NULL, "OUT_OF_MEMORY(0x%08x)", METADATA_EXTRACTOR_ERROR_OUT_OF_MEMORY);
...
int metadata_extractor_set_path(metadata_extractor_h metadata, const char *path)
{
	...
	compare_if_return(!_metadata || !path, "INVALID_PARAMETER(0x%08x)", METADATA_EXTRACTOR_ERROR_INVALID_PARAMETER);
	...
	compare_if_return(_metadata->path == NULL, "OUT_OF_MEMORY(0x%08x)", METADATA_EXTRACTOR_ERROR_OUT_OF_MEMORY);
...
int metadata_extractor_set_buffer(metadata_extractor_h metadata, const void *buffer, int size)
{
	...
	compare_if_return(!_metadata || !buffer || size <= 0, "INVALID_PARAMETER(0x%08x)", METADATA_EXTRACTOR_ERROR_INVALID_PARAMETER);
...
int metadata_extractor_destroy(metadata_extractor_h metadata)
{
	...
	compare_if_return(!_metadata, "INVALID_PARAMETER(0x%08x)", METADATA_EXTRACTOR_ERROR_INVALID_PARAMETER);
...
int metadata_extractor_get_synclyrics(metadata_extractor_h metadata, int index, unsigned long *time_stamp, char **lyrics)
{
	...
	compare_if_return(!_metadata || (!_metadata->path && !_metadata->buffer), "INVALID_PARAMETER(0x%08x)", METADATA_EXTRACTOR_ERROR_INVALID_PARAMETER);
	...
	if (is_string) {
		if ((s_value != NULL) && (strlen(s_value) > 0)) {
			*value = strdup(s_value);
			...
			compare_if_return(*value == NULL, "OUT_OF_MEMORY(0x%08x)", METADATA_EXTRACTOR_ERROR_OUT_OF_MEMORY);
		} else {
			*value = NULL;
		}
	} else {
		...
		*artwork = calloc(1, _artwork_size);
		...
		if ((_artwork_mime != NULL) && (strlen(_artwork_mime) > 0)) {
			*mime_type = strdup(_artwork_mime);
			...
			compare_if_return(*mime_type == NULL, "OUT_OF_MEMORY(0x%08x)", METADATA_EXTRACTOR_ERROR_OUT_OF_MEMORY);
		} else {
			*mime_type = NULL;
		}
...
int metadata_extractor_get_frame(metadata_extractor_h metadata, void **frame, int *size)
{
	...
	compare_if_return(!_metadata || (!_metadata->path && !_metadata->buffer) || !size, "INVALID_PARAMETER(0x%08x)", METADATA_EXTRACTOR_ERROR_INVALID_PARAMETER);
	...
	if ((_frame_size > 0) && (_frame != NULL)) {
		*frame = calloc(1, _frame_size);
		...
		compare_if_return(*frame == NULL, "OUT_OF_MEMORY(0x%08x)", METADATA_EXTRACTOR_ERROR_OUT_OF_MEMORY);
...
int metadata_extractor_get_frame_at_time(metadata_extractor_h metadata, unsigned long timestamp, bool is_accurate, void **frame, int *size)
{
	...
	neq_err_none_free(METADATA_EXTRACTOR_ERROR_OPERATION_FAILED, "METADATA_EXTRACTOR_ERROR_OPERATION_FAILED(0x%08x)", _frame, ret);
